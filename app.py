...
class Libro(db.Model):  # Producto hereda de db.Model
    """
    Definición de la tabla Producto en la base de datos.
    La clase Producto hereda de db.Model.
    Esta clase representa la tabla "Producto" en la base de datos.
    ...
    def __init__(self, title, name, surname, isbn, stock):
        """
        Constructor de la clase Producto.
...
# Definición del esquema para la clase Libro
class LibroSchema(ma.Schema):
    """
    Esquema de la clase Libro.

    Este esquema define los campos que serán serializados/deserializados
    para la clase Libro.
    """
    class Meta:
        fields = ("id", "title", "surname", "name", "isbn", "stock")

libro_schema = LibroSchema()  # Objeto para serializar/deserializar un libro
libros_schema = LibroSchema(many=True)  # Objeto para serializar/deserializar múltiples libros
...
@app.route("/")
def index():
    all_products = Libro.query.all()
    todos = libros_schema.dump(all_products)
...
def get_libros():
    ...
    Retorna un JSON con todos los registros de la tabla de productos.
    """
    all_libros = Libro.query.all()  # Obtiene todos los registros de la tabla de productos
...
def delete_libro(id):
    ...
    """
    libro = Libro.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    db.session.delete(libro)  # Elimina el producto de la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return libro_schema.jsonify(libro)  # Retorna el JSON del producto eliminado

@app.route("/libros", methods=["POST"])  # Endpoint para crear un producto
def create_libro():
    """
    Endpoint para crear un nuevo producto en la base de datos.

    Lee los datos proporcionados en formato JSON por el cliente y crea un nuevo registro de libro en la base de datos.
    Retorna un JSON con el nuevo libro creado.
    """
    title = request.json["title"]  # Obtiene el título del libro del JSON proporcionado
    surname = request.json["surname"]  # Obtiene el apellido del libro del JSON proporcionado
    name = request.json["name"]  # Obtiene el nombre del libro del JSON proporcionado
    isbn = request.json["isbn"]  # Obtiene el isbn del libro del JSON proporcionado
    stock = request.json["stock"]  # Obtiene el stock del libro del JSON proporcionado
  
    new_libro = Libro(title, surname, name, isbn, stock)  # Crea un nuevo objeto Libro con los datos proporcionados
    db.session.add(new_libro)  # Agrega el nuevo libro a la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return libro_schema.jsonify(new_libro)  # Retorna el JSON del nuevo libro creado

@app.route("/libros/<id>", methods=["PUT"])  # Endpoint para actualizar un libro
def update_libro(id):
    """
    Endpoint para actualizar un libro existente en la base de datos.

    Lee los datos proporcionados en formato JSON por el cliente y actualiza el registro del libro con el ID especificado.
    Retorna un JSON con el libro actualizado.
    """
    libro = Libro.query.get(id)  # Obtiene el libro existente con el ID especificado

    # Actualiza los atributos del libro con los datos proporcionados en el JSON
    libro.title = request.json["title"]  # Obtiene el título del libro del JSON proporcionado
    libro.surname = request.json["surname"]  # Obtiene el apellido del libro del JSON proporcionado
    libro.name = request.json["name"]  # Obtiene el nombre del libro del JSON proporcionado
    libro.isbn = request.json["isbn"]  # Obtiene el isbn del libro del JSON proporcionado
    libro.stock = request.json["stock"]  # Obtiene el stock del libro del JSON proporcionado
  
    db.session.commit()  # Guarda los cambios en la base de datos
    return libro_schema.jsonify(libro)  # Retorna el JSON del libro actualizado
